...
async function seedMedia() {
    try {
        ...
        const users = await User_1.default.find().limit(5);
        if (users.length === 0) {
            console.log('⚠️ No users found. Media will be created without uploader.');
        }
        const existingMedia = await Media_1.default.countDocuments();
        ...
        for (let i = 0; i < 20; i++) {
            ...
            const uploadDate = getRandomDate(Math.floor(Math.random() * 90));
            ...
            const medium = {
                ...
                size: baseMedium.size + Math.floor(Math.random() * 100000),
                uploader: user?._id,
        ...
        await Media_1.default.insertMany(mediaToCreate);
        console.log(`✅ Successfully seeded ${mediaToCreate.length} media files`);
        const mediaStats = await Media_1.default.aggregate([
            {
                $group: {
                    _id: '$type',
                    count: { $sum: 1 },
        ...
        const folderStats = await Media_1.default.aggregate([
            {
                $group: {
                    _id: '$folder',
                    count: { $sum: 1 }
        ...
        const totalStorage = await Media_1.default.aggregate([
            {
                $group: {
                    _id: null,
                    totalFiles: { $sum: 1 },
